...
# One column per row of the per-column table in `_designs/PREDICATE_MODEL.md`, so that a
...
#
# PyArrow writes no INT96 statistics. The multi-row-group and dictionary layouts get bounds that
# order the stored bytes as a big-endian signed integer rather than as instants, so pruning against
# them drops rows that match.
