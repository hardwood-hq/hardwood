...
| `UnsupportedOperationException` | A schema column of an unsupported physical type (`INT96`); a refused compression codec (`LZ4`, `LZO`), one whose library is not on the classpath, or one whose native library will not load; a schema shape the writer cannot produce, namely repetition no `LIST` or `MAP` annotation accounts for |
