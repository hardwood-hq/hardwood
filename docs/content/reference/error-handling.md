...
| Exception | When |
|-----------|------|
| `IOException` | Any I/O error: invalid Parquet file (bad magic number, corrupt footer), encrypted files (Parquet Modular Encryption is not supported), local-disk read errors, S3 transport failures (after retry exhaustion — see [Read from S3](../how-to/s3.md)) |
