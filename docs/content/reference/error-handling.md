...
| `IllegalStateException` | Calling `ColumnReader` accessors before `nextBatch()`, calling `nextBatch()` on a reader obtained from `ColumnReaders`, or calling nested-column methods on a flat column; building a row or column reader from a closed `ParquetFileReader`, or while another thread closes it |
