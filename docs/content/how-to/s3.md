...
For the full AWS credential chain (env vars, `~/.aws/credentials` and `~/.aws/config` profiles, SSO, assumed roles, web identity, EC2/ECS instance profile), add the optional `hardwood-aws-auth` module. It pulls in the AWS SDK's `sso`, `ssooidc` and `sts` clients, which the SDK needs for the SSO, assumed-role and web-identity sources:
...
`defaultChain()` delegates to the AWS SDK's `DefaultCredentialsProvider`, which decides the order in which the sources are tried. `SdkCredentialsProviders.fromProfile("my-profile")` binds one named profile from `~/.aws/config` and `~/.aws/credentials` instead.
...
When reading from S3, Hardwood coalesces column chunk reads within each row group into as few HTTP requests as possible (typically 1-2 per row group). Column projection, page-level predicate pushdown, and `head(n)` on an unfiltered read all narrow the byte ranges before coalescing, reducing the amount of data transferred.
