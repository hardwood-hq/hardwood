...
When reading from S3, Hardwood coalesces column chunk reads within each row group into as few HTTP requests as possible. A row group whose column chunks have a page index costs one more request for the index, and a filtered read may add one request per dictionary or bloom filter it checks. Column projection, page-level predicate pushdown, and `head(n)` on an unfiltered read all narrow the byte ranges before coalescing, reducing the amount of data transferred.
