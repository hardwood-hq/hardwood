...
To resolve credentials through the AWS credential chain (environment variables, profile files, container and instance metadata), add the optional `hardwood-aws-auth` module:
...
`defaultChain()` delegates to the AWS SDK's `DefaultCredentialsProvider`, which decides the order in which the sources are tried. `SdkCredentialsProviders.fromProfile("my-profile")` binds one named profile from `~/.aws/credentials` instead. SSO and web-identity profiles need the AWS SDK's `sso`, `ssooidc` and `sts` modules on the classpath; `hardwood-aws-auth` does not bring them.
...
When reading from S3, Hardwood coalesces column chunk reads within each row group into as few HTTP requests as possible. The page index of the columns a read uses is fetched for several row groups at a time, one request for the offset indexes and, on a filtered read, one for the column indexes; a filtered read may add one request per dictionary or bloom filter it checks. Column projection, page-level predicate pushdown, and `head(n)` on an unfiltered read all narrow the byte ranges before coalescing, reducing the amount of data transferred.

Without a row limit, the reader fetches all projected (and filtered) column data for an entire row group when any column enters it. For large row groups (128 MB–1 GB is typical), this can transfer significant data even if the consumer stops reading early. To minimize data transfer for partial reads, set `head(n)` on the `buildRowReader()` builder; this truncates each column's fetch to only the pages covering the needed rows. With a filter, `head(n)` counts matching rows, which are not known before the pages are decoded, so it does not narrow the fetch.

### Measuring Fetch Cost

Each `S3InputFile` tracks the network I/O it has performed, which is useful for confirming that column projection, predicate pushdown, and `head(n)` reduced transfer — and for estimating S3 cost (GET requests plus bytes transferred):
...
Anonymous (unsigned) requests and requester-pays buckets are not supported — see the [S3 Reference](../reference/s3.md#not-supported) for details.
