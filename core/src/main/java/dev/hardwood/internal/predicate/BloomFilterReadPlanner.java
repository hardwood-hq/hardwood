...
public final class BloomFilterReadPlanner {
    ...
    /// @param readability the file's own bounds readability, the value the real pruning pass
    ///        decides with — the dry run prunes on the same statistics, so it records the same
    ///        columns
    public static BloomFilterReadPlan plan(ResolvedPredicate predicate, List<RowGroup> rowGroups,
            BoundsReadability readability) throws IOException {
        if (predicate == null) {
        ...
        for (int rowGroupIndex = 0; rowGroupIndex < rowGroups.size(); rowGroupIndex++) {
            ...
            candidates.addAll(planRowGroup(rowGroupIndex, rowGroup, predicate, readability));
        }
    ...
    private static List<BloomCandidate> planRowGroup(int rowGroupIndex, RowGroup rowGroup,
        ...
        // No log context: the real pruning pass decides these same row groups moments later
        // and raises each statistics warning there. Reporting here would double every one.
        RowGroupFilterEvaluator.decideRowGroup(predicate, rowGroup, recording, null, null, readability);
        List<BloomCandidate> candidates = new ArrayList<>();
