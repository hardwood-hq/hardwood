...
class CrcValidator {
    ...
    ///
    /// Which column the page belongs to is the caller's to say: every path that reaches
    /// here does so under a file, row group and column that the failure is prefixed with.
    static void assertCorrectCrc(int expectedCrc, ByteBuffer pageData) {
        ...
        if (actualCrc != expectedCrc) {
            throw new ParquetReadException("CRC mismatch: expected " + Integer.toHexString(expectedCrc)
