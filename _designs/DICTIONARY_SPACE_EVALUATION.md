...
a projected top-level field and its resolved comparison is byte-exact.
`BYTE_STRING` values and fixed-width decimals have one byte encoding per value,
so dictionary-entry equality is sound for them. Variable-length decimals are
not eligible because a writer may represent the same number with different
amounts of sign-extension padding.
...
Eligible equality compares byte sequences exactly. It is independent of the
signed or unsigned ordering used by range predicates. Nulls follow the existing
