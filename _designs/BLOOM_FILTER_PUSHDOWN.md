...
- For `FLOAT` / `DOUBLE`, the probe value is not `NaN`. Bloom hashing is over the raw IEEE-754 bits, which distinguish `-0.0` from `+0.0` and every NaN payload, whereas value equality treats `NaN != NaN` across payloads. Probing a NaN by raw bits could prove a value absent that an equal stored payload would match, so NaN is never pruned (statistics still apply). Signed zeros are safe: the matcher's `Float.compare` / `Double.compare` distinguish `-0.0` from `+0.0` exactly as the raw-bit hash does (#829). `hash(float)` / `hash(double)` reuse the `int` / `long` primitives via `floatToRawIntBits` / `doubleToRawLongBits`, matching parquet-java's `putFloat` / `putDouble`.
