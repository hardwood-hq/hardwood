...
- The column's physical type is one of `INT32`, `INT64`, `FLOAT`, `DOUBLE`, `BYTE_ARRAY`, or `FIXED_LEN_BYTE_ARRAY` — the types for which the resolved predicate value maps unambiguously to the value's plain encoding, which is what the writer hashed. This corresponds to the resolved leaf types `IntPredicate` / `IntInPredicate` (→ `XxHash64.hash(int)`), `LongPredicate` / `LongInPredicate` (→ `XxHash64.hash(long)`), `FloatPredicate` (→ `XxHash64.hash(float)`), `DoublePredicate` (→ `XxHash64.hash(double)`), `DoubleInPredicate` (→ `XxHash64.hash(float)` per probe on a `FLOAT` column, `XxHash64.hash(double)` on a `DOUBLE` column), and `BinaryPredicate` / `BinaryInPredicate` (→ `XxHash64.hash(byte[])`).
- For `FLOAT` / `DOUBLE` equality, the probe value is not `NaN`. Bloom hashing is over the raw IEEE-754 bits and the record/batch matchers use total-order comparison (`Float.compare` / `Double.compare`), so both distinguish `-0.0` from `+0.0` and signed zeros are safe to probe at their exact bit width. `NaN` values are never pruned by bloom filters because raw-bit hashing distinguishes distinct NaN payloads that total-order equality treats as equal, so a bloom miss cannot prove absence. Statistics prune a `±0.0` probe, and prune a `NaN` equality probe only where the unit records a `nan_count` of zero, because a usable min/max pair covers non-NaN values only. `hash(float)` / `hash(double)` reuse the `int` / `long` primitives via `floatToRawIntBits` / `doubleToRawLongBits`, matching parquet-java's `putFloat` / `putDouble`. For the floating-point `IN` leaf, a `NaN` probe disables the bloom check for the whole list (one possibly-present value keeps the row group), while a `±0.0` probe is hashed at its own bit width — the cast round trip preserves the sign bit, so the list path needs no zero carve-out. A `FLOAT`-column probe with no exact `float` representation (e.g. `0.1`) cannot equal any stored float widened to double, so it is provably absent without consulting the filter.
- The column chunk actually carries a bloom filter (`bloomFilterOffset != null`).

Everything else falls through to statistics only: `NOT_EQ` and range operators (a bloom filter cannot prove them), `FLOAT16` / `BOOLEAN` columns, `NaN` floating-point equality, and any column without a bloom filter. The `signed` flag on `BinaryPredicate` (FLBA decimals) does not affect hashing — the writer hashes the raw value bytes regardless of sign interpretation — so signed and unsigned binary equality are both eligible.
...
A `bloomFilterOffset` that is present but non-positive cannot name a real filter (it points at or before the file's magic header); it is logged at `WARNING` and treated as "no filter" — conservatively keeping the row group — rather than throwing. A header naming an algorithm, hash or compression variant this version does not implement is treated the same way, on the `UnsupportedOperationException` `BloomFilterHeaderReader` raises for it: the file is correct and only the pruning shortcut is lost. An `IOException` while reading a filter the footer declared is treated as corruption and surfaced as an `UncheckedIOException` carrying the file name (fail-early), consistent with `RowGroupIndexBuffers.fetch`.
...
- **Evaluator** (`BloomFilterPushDownTest`) against `bloom_filter_test.parquet` (single row group, 64 rows; bloom filters on `id` INT64 `0..63`, `code` INT32 `0,3,…,189`, `name` STRING `""`…`"x"*63`, `price` FLOAT `0,2,…,126`, `ratio` DOUBLE `0,0.5,…,31.5`, `dec` DECIMAL(38,0) stored as FLBA(16) `0,2,…,126`, `ts` TIMESTAMP(us,UTC) at even-second offsets from `2024-01-01`, `sparse` INT64 `0,1000,…,63000`; `value` INT64 has none):
  ...
  - `eq("price", -0.0f)` — `+0.0f` is stored; matchers and bloom hashes both distinguish its sign from `-0.0f`, so the bloom filter drops it.
  - `eq("value", …)` — no bloom filter; falls back to statistics only.
  - `in("code", [1, 2])` (INT32), `in("sparse", [1, 2])` (INT64, `sparse` = `0,1000,…,63000`), `in("price", [1.0, 3.0])` (FLOAT), `in("ratio", [0.25, 0.75])` (DOUBLE), and `inStrings("name", ["w", "q"])` (binary) — all values absent; dropped. The same lists with one present value (`3` / `1000` / `2.0` / `0.5` / `"xx"`) are kept. `eq("sparse", 1)` covers the INT64 `eq` drop that `id` (contiguous) cannot.
