...
| Condition | Path |
|---|---|
| Every kept row group can take a per-page row mask on every projected column (`RowGroupIterator.canFastSkipAllRowGroups`) | `setTailSkip(residue)` gives work item 0 the matching range `[residue, numRows)`. Pages wholly before it are not fetched, and the first page it touches is trimmed to it, so the workers assemble exactly `n` rows |
