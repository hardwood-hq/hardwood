...
- **`NULL`.** A conforming file stores only nulls there, so no comparison matches. A file that stores values anyway is compared by what it stores under equality. A footer that gives `UNKNOWN` beside a `converted_type` annotates the column with the converted type, so parquet-java's `INTERVAL` columns take the `INTERVAL` row.
