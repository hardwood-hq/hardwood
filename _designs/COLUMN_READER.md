...
A **generation** counter numbers the steps the views share: it increments for every batch, and once more the first time the end of the input is reached. A read in which pruning dropped every row group gets a scan with no cursors (`ColumnScan.empty`), whose first `advance()` returns `false` and starts no worker.

**View advance.** Each view records the generation it last adopted. `ColumnReader.nextBatch()` advances the scan when the view has already adopted the current generation, adopts the step a sibling advanced to when the view is one generation behind, and throws `IllegalStateException` when it is further behind, since adopting would skip a batch. `ColumnReaders.nextBatch()` advances the scan and has every member adopt the new step. Calling `nextBatch()` on each member in turn therefore moves the group once per turn, a member cannot pair rows from different steps, and a member left behind fails instead of silently dropping batches. A member never advanced does not hold the others back. `ColumnReaders.getRecordCount()` reads the scan's current step, whichever view advanced it.
...
**Closing.** Closing any view of a group closes the scan: it closes every cursor, then releases the iterator even when a cursor's teardown fails. Afterwards `nextBatch()` on any view of the group throws `IllegalStateException`. Close is idempotent.
