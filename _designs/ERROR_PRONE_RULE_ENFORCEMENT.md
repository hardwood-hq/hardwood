...
- **Per-job closure** (`main-build.yml`, `pr-build.yml`): each job opens by building
  its own dependency closure with `-Dquick … -am`, and no job downloads an artifact.
  Every module declares `hardwood-error-prone-checks` as an ordinary dependency, so
  `-am` pulls it into the closure and installs it even though `-Dquick` deactivates
  `qa` for that build.
