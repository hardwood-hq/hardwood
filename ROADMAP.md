...
- [x] Dictionary-space binary predicates for row and exact column reads: lazily decide referenced entries for equality, inequality, ranges, and membership, then match encoded rows by entry ID with exact fallback for plain and cross-chunk rows (#859; see `_designs/DICTIONARY_SPACE_EVALUATION.md`)
- [ ] Extend dictionary-space evaluation to fixed-width physical types (#859)
