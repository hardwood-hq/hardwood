...
**Status: In progress.** Tracking issue: #1291. Stages 1–20a, 26a, 27, 28, 31 and 36 shipped under #9.
...
`_designs-legacy/WRITER_NESTED.md` (delivery stage 5) and implemented structs → lists → maps in
stages 6–8.
...
`TESTING.md#differential-testing` and the writer runs it in reverse.
...
1. **Strict-reader interop gate (primary)**: hardwood writes a file, parquet-java reads
   ...
   already lives. Settled in [WRITER_INTEROP_GATE.md](../_designs-legacy/WRITER_INTEROP_GATE.md).
...
| 5 | **Nested write design**: the shredding model (rep/def-level computation from struct / list / map nesting), the nested `ColumnBatch` input contract (per-layer validity + offsets, the write-side analog of `getLayerValidity` / `getLayerOffsets`), and `FileSchema.Builder` group / repeated-field support. Produces `_designs-legacy/WRITER_NESTED.md`, the reference the shredding increments implement against. | Spike | The nested write contract is settled | 6.3 (design) | [x] |
...
| 9 | Dictionary encoding (`INT32`): dictionary page + `RLE_DICTIONARY` indices + plain fallback, exercised on nullable and nested columns so the level + dictionary-index page layout is proven together. Settled in `_designs-legacy/WRITER_DICTIONARY.md`. | Dimension | Dictionary column-chunk layout proven, incl. nulls and nesting | 2.2 | [x] |
...
| 37 | **`encoding_stats` in the column metadata** (#1083): `ColumnMetaDataWriter` serializes optional field 13 — `list<PageEncodingStats>`, one `(page_type, encoding, count)` per combination — for every column chunk. It is the field a reader needs to know whether a chunk's dictionary covers all of the chunk's values, so a hardwood-written dictionary chunk gets dictionary-based row-group pruning, from Hardwood and parquet-java: `RowGroupDictionaryFilterSource` treats a chunk with no `encoding_stats` as ineligible, because a dictionary that describes only a prefix cannot prove a value absent. `hardwood dive` reads the same field for its per-page `Encoding` column and its `Chunk encodings` line, both of which report the writer's page and chunk encodings. The coverage question the field exists to answer is one this writer answers by construction: since 18 the encoding is chosen once per chunk from the fully buffered row group, so a chunk is `RLE_DICTIONARY` or `PLAIN` throughout and a dictionary page is written only for a chunk that uses it. The entries follow from the choice already made — a `DICTIONARY_PAGE` entry where there is one, a `DATA_PAGE` entry over the pages that chunk wrote — with nothing to track and no partial coverage to represent. Covered by a round trip between the writer and that reader: `RowGroupDictionaryFilterSource` finding the dictionary of a hardwood-written chunk, and finding none for a `PLAIN` one. | Layer | Dictionary pruning on produced files | — | [x] |
